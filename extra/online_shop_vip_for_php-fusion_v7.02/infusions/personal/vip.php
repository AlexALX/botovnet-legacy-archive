...
$res = dbquery("SELECT * FROM ".DB_PREFIX."game_vip WHERE uid='".$userdata['user_id']."'");
...
if (isset($_POST['save'])) {
	...
	if ($error!="") {
		...
		$res = dbquery("UPDATE ".DB_PREFIX."game_vip SET name='".$name."', type='".$type."', password='".$password."', options='".$options."' WHERE uid='".$userdata['user_id']."'");
...
<td class='tbl1'>".get_to_time($data['time']-($data['ftime']>0?$data['ftime']:time()),false,$data['server'])."</td>
</tr>";
}
echo "<tr>
...
<td class='tbl1'>".get_flags($data['flags'])."</td>         // <a href='".FUSION_SELF."?action=edit'><img src='".VIP_IMAGES."edit.png' alt='Редактировать' title='Редактировать'></a>
</tr>*/ ."
<tr>
...
if ($data['server']==0||$data['server']>1) {

add_to_head("<link rel='stylesheet' href='".INCLUDES."jscripts/colorpicker/css/bootstrap-colorpicker.css' type='text/css' />");
add_to_head("<script src='".INCLUDES."jscripts/colorpicker/js/bootstrap-colorpicker.min.js' type='text/javascript'></script>");

echo "<tr>
<td class='tbl2' colspan='2' align='center'><b>Подбор цвета</b> (rust)</td>
</tr>
<tr>
<td class='tbl1' colspan='2' align='center'>Установка цвета ника:<br><input type='text' style='width:170px' class='textbox' id='colortxt2' value='/vip color #5367ce' /><br>
Установка цвета сообщения:<br><input type='text' class='textbox' style='width:170px' id='colortxt3' value='/vip textcolor #5367ce' /><br>
<input type='hidden' style='width:170px' id='colortxt' value='#5367ce' /><div id='color'></div>
<script type='text/javascript'>
$(function(){
    $('#colortxt').colorpicker({inline:true, container:'#color'}).on('changeColor.colorpicker', function(event){
 		$('#colortxt2').val('/vip color ' + event.color.toHex());
 		$('#colortxt3').val('/vip textcolor ' + event.color.toHex());
	});
});
</script>
<span class='small2'>Используйте эти команды для установки цвета в игре.</color></td>
</tr>";
}
