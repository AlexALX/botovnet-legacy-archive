...
if (!checkrights("I") || !defined("iAUTH") || $_GET['aid'] != iAUTH) { redirect(VIP_BASEDIR); };
...
if ($_GET['action']=="edit") {
if (!isset($_GET['id'])||!isnum($_GET['id'])) redirect(VIP_BASEDIR."admin.php".$aidlink);

$res = dbquery("SELECT v.*, u.user_name, a.user_name AS admin_name FROM ".DB_PREFIX."csvips v LEFT JOIN ".DB_PREFIX."users u ON u.user_id=v.uid LEFT JOIN ".DB_PREFIX."users a ON a.user_id=v.aid WHERE vid='".$_GET['id']."'");
...
if (isset($_POST['save'])) {
	...
	if ($error!="") {
		...
		if ($_GET['action']=="new") {
			$res = dbquery("INSERT INTO ".DB_PREFIX."csvips VALUES('','".$uid."','".$type."','".$name."','".$password."','".$time."','".$server."','".$flags."','".$options."','".$userdata['user_id']."','".$status."','".time()."','0')");
			...
			$res = dbquery("UPDATE ".DB_PREFIX."csvips SET name='".$name."', type='".$type."', password='".$password."', options='".$options."', flags='".$flags."',time=".($add!=0?"time".$typ.$add:"'".$time."'").",status='".$status."',uid='".$uid."',server='".$server."', ftime='".$ftime."' WHERE vid='".$_GET['id']."'");
...
foreach ($servers as $key=>$serv) {
	echo "<option value='".$key."'".($data['server']==$key?" selected":"").">".$serv."</option>";
}
echo "</select></td>
</tr>
<tr>
...
closetable();
	...
	redirect(VIP_BASEDIR."admin.php".$aidlink);
}
...
if (isset($_POST['save'])) {
	...
	$res = dbquery("UPDATE ".DB_PREFIX."csvips_set SET enabled='".$enabled."', free_vip='".$free_vip."', free_vipt='".$free_vipt."', servers='".$servers."'");
	...
	if (isset($_POST['time'])&&isnum($_POST['time'])) {
		...
		$where = "";
		if (isset($_POST['servers_add'])&&is_array($_POST['servers_add'])&&count(get_servers("",true))>count($_POST['servers_add'])) {
			$where = " AND ("; $i = 0;
		...
		if ($add>0) $res = dbquery("UPDATE ".DB_PREFIX."csvips SET time=time".$typ.$add." WHERE `time`>'0' AND `time`>'".time()."'".$where);
	}
...
foreach ($servers as $key=>$serv) {
	echo "<option value='".$key."'".(in_array($key,$servs)?" selected":"").">".$serv."</option>";
}
echo "</select></td>
</tr>
<tr>
...
$res = dbquery("SELECT * FROM ".DB_PREFIX."csvips_pays WHERE pid='".$_GET['pid']."'");
...
if (isset($_POST['save'])) {
	...
	if ($vid==""||$ammount==""||!isnum($vid)||!isnum($status)||$vid==0||$server==""||$time==""||$amtype==""||!isnum($server)||!isnum($amtype)) {
		if ($error!="") $error .= "<br>";
		$error .= "Не все обязательные поля были заполнены.";
	}
	...
	if ($error != "") {
		...
		if (isset($_GET['add'])) {
			$res = dbquery("INSERT INTO ".DB_PREFIX."csvips_pays VALUES ('','".$vid."','".($time!=0?$time:time())."','".$type."','".$ammount."','".$amtype."','','".$date."','','','','".$status."','".$desc."','".$server."','".$vtime."');");
			redirect(FUSION_SELF.$aidlink."&action=pays&view".(isset($_GET['id'])&&isnum($_GET['id'])?"&id=".$_GET['id']:"")."&pid=".mysql_insert_id()."&status=saved");
		} else {
			$res = dbquery("UPDATE ".DB_PREFIX."csvips_pays SET vid='".$vid."', time='".$time."', date='".$date."', status='".$status."', type='".$type."', ammount='".$ammount."', ammount_type='".$amtype."', `desc`='".$desc."', server='".$server."', vip_time='".$vtime."' WHERE pid='".$_GET['pid']."';");
...
if (isset($_GET['add'])) $data = array("pid"=>" Новый","vid"=>(isset($_GET['id'])&&isnum($_GET['id'])?$_GET['id']:"0"),"time"=>"0","date"=>"0","status"=>"0","ammount"=>"0.00","ammount_type"=>"0","desc"=>"","server"=>"0","vip_time"=>"0","type"=>"2");
else $data = dbarray($res);
...
<td class='tbl1'><select class='textbox' name='vip_time'>
  <option value='0'".($data['vip_time']=='0'?" selected":"").">2 Недели</option>
  <option value='1'".($data['vip_time']=='1'?" selected":"").">1 Месяц</option>
  <option value='2'".($data['vip_time']=='2'?" selected":"").">2 Месяца</option>
  <option value='3'".($data['vip_time']=='3'?" selected":"").">3 Месяца</option>
  <option value='4'".($data['vip_time']=='4'?" selected":"").">6 Месяцев</option>
...
$res = dbquery("SELECT o.*,v.uid,u.user_name FROM ".DB_PREFIX."csvips_pays o LEFT JOIN ".DB_PREFIX."csvips v ON v.vid=o.vid LEFT JOIN ".DB_PREFIX."users u ON v.uid=u.user_id WHERE o.pid='".$_GET['pid']."'");
...
if (isset($_GET['pay'])&&$data['status']==0) {
	$res2 = dbquery("UPDATE ".DB_PREFIX."csvips_pays SET type='2', date='".time()."', `status`='1' WHERE pid='".$data['pid']."'");
...
if (isset($_GET['del'])) {
	if (!isset($_GET['pid'])||!isnum($_GET['pid'])) redirect(FUSION_SELF.$aidlink);
	$res = dbquery("DELETE FROM ".DB_PREFIX."csvips_pays WHERE pid='".$_GET['pid']."'");
...
$items_per_page = 20;
$res = dbquery("SELECT o.*,v.uid,u.user_name FROM ".DB_PREFIX."csvips_pays o LEFT JOIN ".DB_PREFIX."csvips v ON v.vid=o.vid LEFT JOIN ".DB_PREFIX."users u ON v.uid=u.user_id ".(isset($_GET['id'])&&isnum($_GET['id'])?"WHERE o.vid='".$_GET['id']."' ":"")."ORDER BY time DESC LIMIT $rowstart,$items_per_page");
...
if (dbrows($res)) {
	...
	$rows = dbcount("(vid)", "csvips_pays", (isset($_GET['id'])&&isnum($_GET['id'])?" vid='".$_GET['id']."'":""));
...
if (isset($_GET['del'])) {
	...
	$res = dbquery("DELETE FROM ".DB_PREFIX."csvips WHERE vid='".$_GET['id']."'");
	$res = dbquery("DELETE FROM ".DB_PREFIX."csvips_pays WHERE vid='".$_GET['id']."'");
	$res = dbquery("DELETE FROM ".DB_PREFIX."csvips_ses WHERE vid='".$_GET['id']."'");
...
if (!isset($_GET['rowstart']) || !isnum($_GET['rowstart'])) $rowstart = 0; else $rowstart = $_GET['rowstart'];
$items_per_page = 20;
$res = dbquery("SELECT v.*,u.user_name FROM ".DB_PREFIX."csvips v LEFT JOIN ".DB_PREFIX."users u ON u.user_id=v.uid ORDER BY v.time='0' DESC, v.time DESC LIMIT $rowstart,$items_per_page");

echo "<center>[ <a href='".FUSION_SELF.$aidlink."&action=new'>Добавить нового</a> ]</center><br>";

if (dbrows($res)) {

	$rows = dbcount("(vid)", "csvips");
...
if (dbrows($res)) {
	...
	$costs = dbarray(dbquery("SELECT (SELECT sum(ammount) FROM `".DB_PREFIX."csvips_pays` WHERE ammount_type='0' AND type='0' AND status='1') AS uasum,(SELECT sum(ammount) FROM `".DB_PREFIX."csvips_pays` WHERE ammount_type='1' AND type='0' AND status='1')  AS rusum"));
	echo "<table align='center' cellpadding='0' cellspacing='0' width='100%'>
	<tr>
	<td width='40%'>Всего випов: ".dbcount("(vid)","csvips")."</td>
	<td width='60%'>Всего платежей: ".dbcount("(pid)","csvips_pays")."</td>
	</tr>
	<tr>
	<td>Активных: ".dbcount("(vid)","csvips"," status='1' AND (time>'".time()."' or time='0')")."</td>
	<td>Оплаченых: ".dbcount("(pid)","csvips_pays"," status='1'")."</td>
	</tr>
	<tr>
	<td>Замороженных: ".dbcount("(vid)","csvips"," status='0'")."</td>
	<td>Неудачных: ".dbcount("(pid)","csvips_pays"," status='2'")."</td>
	</tr>
	<tr>
	<td>Заблокированных: ".dbcount("(vid)","csvips"," status='2'")."</td>
	<td>Отменённых: ".dbcount("(pid)","csvips_pays"," status='3'")."</td>
	</tr>
	<tr>
	<td>Получено средств: ".ceil($costs["uasum"])." UAH и ".ceil($costs["rusum"])." RUB</td>
	<td>Ожидающих: ".dbcount("(pid)","csvips_pays"," status='0'")."</td>
	</tr>
	</table>";
