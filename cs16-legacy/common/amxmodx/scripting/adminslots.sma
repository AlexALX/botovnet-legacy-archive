...
*
* This plugin was edited for Botov-NET Project (VIP system)
...
public plugin_init()
{
	register_plugin("Slots Reservation", AMXX_VERSION_STR, "AMXX Dev Team / AlexALX")
	...
	//format(g_cmdLoopback, 15, "adrf%c%c%c%c", random_num('A', 'Z'), random_num('A', 'Z'), random_num('A', 'Z'), random_num('A', 'Z'))
...
public check_slot(id)
{
	...
	if (access(id, ADMIN_RESERVATION) || (get_sysvip_flags(id) & VIP_FLAG_A) || (players <= limit) || is_user_bot(id))
	{
