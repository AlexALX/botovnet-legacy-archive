 ...
 * Waterman - UP Plugin for New Version =)
 *
 * It is edited for Botov-NET project
 ...
 */
   ...
   //  #include <zombieplague>
    ...
    #define Plugin   "Bio: Squeak Grenade"
    #define Version  "1.0.6"
    #define Author   "Arkshine / Waterman / AlexALX"
    ...
    // --| Snark trail.
    #define TRAIL_LIFE        20    // Life
    #define TRAIL_WIDTH       4     // Width
    #define TRAIL_RED         10//10    // Red
    #define TRAIL_GREEN       10//224   // Red
    #define TRAIL_BLUE        224//10    // Green
    ...
    /* - -
     |  CONSTANTS  |
        ...
        new const gWeaponIndex      = CSW_FLASHBANG;
    ...
    /* - - - -
     |  OTHERS STUFFS  |
        ...
        new gPlayerBuy[ MAX_CLIENTS + 1 char ];
        new Float:gPlayerBuyTime[ MAX_CLIENTS + 1 char ];
        new Float:gPlayerRegenTime[ MAX_CLIENTS + 1 char ];
    ...
    public plugin_init ()
    {
        ...
        pCvarAmmo          = register_cvar( "wpn_sg_ammo"           , "1"   );
        CostSnark          = register_cvar( "bio_costsnark"           , "1000"   );
        pCvarRefireRate    = register_cvar( "wpn_sg_refire_rate"    , "0.3" );
        pCvarHealth        = register_cvar( "wpn_sg_health"         , "15"  );
        pCvarVelocity      = register_cvar( "wpn_sg_velocity"       , "200" );
        pCvarDamagePop     = register_cvar( "wpn_sg_damage_pop"     , "5"   );
        pCvarDamageRadius  = register_cvar( "wpn_sg_damage_radius"  , "40"  );
        pCvarGravity       = register_cvar( "wpn_sg_gravity"        , "0.5" );
        pCvarFriction      = register_cvar( "wpn_sg_friction"       , "0.5" );
        pCvarDetonateDelay = register_cvar( "wpn_sg_detonate_delay" , "13"  );
        pCvarFieldOfView   = register_cvar( "wpn_sg_fov"            , "0"   );
        pCvarShowTrail     = register_cvar( "wpn_sg_show_trail"     , "1"   );
        SnarkInfect        = register_cvar( "bio_infsnark"          , "250" );
        SnarkKilled        = register_cvar( "bio_killedsnark"       , "500" );
        SnarkKill          = register_cvar( "bio_killsnark"         , "750" );
	...
	register_clcmd( "say /buy_snarks", "BuySnarks" );
	register_clcmd( "say_team /buy_snarks", "BuySnarks" );
	register_clcmd( "say buy_snarks", "BuySnarks" );
	register_clcmd( "say_team buy_snarks", "BuySnarks" );
	register_clcmd( "/buy_snarks", "BuySnarks" );
	register_clcmd( "buy_snarks", "BuySnarks" );
	register_clcmd( "say /buy_snark", "BuySnarks" );
	register_clcmd( "say_team /buy_snark", "BuySnarks" );
	register_clcmd( "say buy_snark", "BuySnarks" );
	register_clcmd( "say_team buy_snark", "BuySnarks" );
	register_clcmd( "/buy_snark", "BuySnarks" );
	register_clcmd( "buy_snark", "BuySnarks" );
	register_dictionary("squeakgrenade.txt");
	...
	register_concmd("amx_snark", "cmd_give", ADMIN_RCON, "<nick or #userid or @all>")
	...
	//set_task(1.0, "CheckWeapon", 777, _ , _ , "b")
    ...
    public CheckWeapon()
    {
	  new players[32], num
	 	...
	 	if ( is_user_zombie( i ) != 1)
        	{
            		CheckAndRemoveWeapon ( i );
        	} else {
		if(task_exists(698 + i)) remove_task(698 + i)
		}
		}
...
{
	register_native("has_snark","native_has_snark",1)
	register_native("reset_snark","native_reset_snark",1)
	register_native("set_snark","native_set_snark",1)
	register_native("noregen","native_noregen",1)
}

public native_has_snark(id) {
	return gHasSnark { id };
}

public native_reset_snark(id) {
	if (!is_user_connected(id))
		return PLUGIN_HANDLED;
	CheckAndRemoveWeapon(id)
	//gHasSnark    { id } = 0;
	//gWeaponActive{ id } = false;
	//gJustThrown  { id } = false;
	//gPlayerAmmo  { id } = 0;
	return PLUGIN_CONTINUE;
}

public native_set_snark(id) {
	if (!is_user_alive(id))
		return PLUGIN_HANDLED;
	gHasSnark{ id } = true;
	give_item(id,"weapon_flashbang");
	cs_set_user_bpammo( id, CSW_FLASHBANG, gPlayerAmmo { id } )
	return PLUGIN_CONTINUE;
}
...
public event_gamestart() {
	...
	for (new i=0;i<=33;i++) {
		if (is_user_connected(i) && is_user_alive(i) && is_user_zombie(i)) {
			set_task(0.5,"give_snark",i)
		}
	}
...
public give_snark(id) {

	if (!is_user_alive(id) || !is_user_zombie(id) || !game_started())
		return PLUGIN_HANDLED;
	...
	if (gPlayerAmmo { id } != 0) {
		...
		gPlayerBuy{ id } = gPlayerAmmo { id }
		emit_sound ( id, CHAN_VOICE , random_float ( 0.0, 1.0 ) <= 0.5 ? gSnarkHunt2Sound : gSnarkHunt3Sound, VOL_NORM, ATTN_NORM, 0, PITCH_NORM );
		//engclient_cmd( id, "wait;weapon_knife" );
		set_task(0.1,"give_snark_knf",id)
		sel_modelent2(id,1)
	} else {
		gPlayerBuy{ id } = 0
	}
...
public newround() {
	...
	for (new id=0;id<=33;id++) {
		if (is_user_connected(id)) {
			gPlayerBuy  { id } = 0;
			gPlayerBuyTime[id] = 0.0;
			gPlayerRegenTime[id] = 0.0;
			//remove_task(id+12454);
		}
	}
...
public restartround() {
    ...
    public CheckAndRemoveWeapon ( const Player )
    {
        if ( user_has_weapon( Player, gWeaponIndex ) && gHasSnark { Player }  )
        {
            RemoveWeapon ( Player, get_user_weapon( Player ) == gWeaponIndex ? get_pdata_cbase( Player, m_pActiveWeapon ) : find_ent_by_owner ( -1, gWeaponCommand, Player ) );
...
public cmd_give(id, level, cid)
{
			...
			for(new i = 1; i < 33; i++) {
				if (is_user_connected(i)) {
					//if (is_user_alive(i)) {
						...
						CSqueak_GiveWeapon(i)
					//}
				}
			}
			log_amx("^"%s<%d><%s><>^" give snark to all", name, get_user_userid(id), authid)

			show_activity_key("SNARK_7A", "SNARK_8A", name)
		}
	} else {
	...
	log_amx("^"%s<%d><%s><>^" give snark to ^"%s<%d><%s><>^"", name, get_user_userid(id), authid, name2, userid2, authid2)

	show_activity_key("SNARK_7", "SNARK_8", name, name2)
...
stock is_alien(id) {
	if (is_user_zombie(id) && get_class_id("Alien") == get_user_class(id))
		return true;
	return false;
}

public Float:native_noregen(id)
	return gPlayerRegenTime[id];

public BuySnarks ( Player )
{
	...
	if (!game_started()) {
		client_print(Player, print_chat, "%L",Player,"SNARK_NOSTARTED")
		return PLUGIN_CONTINUE;
	}

	if (!is_user_alive(Player)) {
		client_print(Player, print_chat, "%L",Player,"SNARK_ALIVE")
		return PLUGIN_CONTINUE;
	}

	if (!is_user_zombie(Player)) {
		client_print(Player, print_chat, "%L",Player,"SNARK_HUMAN")
		return PLUGIN_CONTINUE;
	}

	if (is_alien(Player)) {
		new cost = (get_vip_flags( Player ) & VIP_FLAG_C ? 100 : 125);

		if (gPlayerAmmo{ Player } >= 1 && !is_user_vip(Player)) {
			client_print(Player, print_chat, "%L",Player,"SNARK_VIPMAX")
			client_print(Player, print_chat, "%L",Player,"SNARK_VIP2")
			return PLUGIN_CONTINUE;
		}
		...
		if (gPlayerBuyTime[Player] > gametime) {
			client_print(Player, print_chat, "%L",Player,"SNARK_TIME", gPlayerBuyTime[Player] - gametime)
			return PLUGIN_CONTINUE;
		}

		if (gPlayerAmmo{ Player } >= 2) {
			client_print(Player, print_chat, "%L",Player,"SNARK_MAX")
			return PLUGIN_CONTINUE;
		}

		if(get_user_health( Player )<=cost ) {
			client_print(Player, print_chat, "%L",Player,"SNARK_HP", cost)
			return PLUGIN_CONTINUE;
		}

		gPlayerBuyTime[Player] = gametime + 30.0;
		gPlayerRegenTime[Player] = gametime + 4.0;

		CSqueak_GiveWeapon( Player )
		set_user_health( Player, get_user_health( Player )-cost )
		client_print(Player, print_chat, "%L",Player,"SNARK_BUY2")
		/*remove_task(Player+12454);
		remove_task(Player+12455);
		set_task(30.0,"resetbuytime",Player+12454)
		set_task(4.0,"restoreregen",Player+12455) */
	} else {
		new cost = (get_vip_flags( Player ) & VIP_FLAG_C ? floatround(get_pcvar_num(CostSnark) * 0.8) : get_pcvar_num( CostSnark ));

		if(cs_get_user_money_ul( Player )<cost ) {
			client_print(Player, print_chat, "%L",Player,"SNARK_MONEY", cost)
			return PLUGIN_CONTINUE;
		}
		...
		if (gPlayerBuy{ Player } >= 3) {
			client_print(Player, print_chat, "%L",Player,"SNARK_ROUND")
			return PLUGIN_CONTINUE;
		}
		CSqueak_GiveWeapon( Player )
		cs_set_user_money_ul( Player, cs_get_user_money_ul( Player )-cost )
		client_print(Player, print_chat, "%L",Player,"SNARK_BUY")
		gPlayerBuy{ Player }++
	}
	return PLUGIN_CONTINUE;
...
/*
public resetbuytime(Player)
	if (is_user_connected(Player-12454)) gPlayerBuyTime{ Player-12454 } = false;

public restoreregen(Player)
	if (is_user_connected(Player-12455)) gPlayerRegenTime{ Player-12455 } = false;
*/
    public client_connect ( Player )
    {
        ...
        gPlayerBuy  { Player } = 0;
        gPlayerBuyTime[Player] = 0.0;
        gPlayerRegenTime[Player] = 0.0;
    }
...
#define G_PICKUP_SND	"items/9mmclip1.wav"
    ...
    public CSqueak_GiveWeapon( const id )
    {
    	if(task_exists(698 + id)) remove_task(698 + id)

	if (!is_user_zombie(id) || !is_user_alive(id) || !game_started()) {
		gPlayerAmmo{ id } = gPlayerAmmo{ id } + get_pcvar_num(pCvarAmmo)
		return PLUGIN_CONTINUE;
	}
	...
	if (user_has_weapon(id,CSW_FLASHBANG)) {
		cs_set_user_bpammo( id, CSW_FLASHBANG, cs_get_user_bpammo(id, CSW_FLASHBANG)+get_pcvar_num(pCvarAmmo) )
		...
		give_item(id,"weapon_flashbang");
		cs_set_user_bpammo( id, CSW_FLASHBANG, get_pcvar_num(pCvarAmmo) )
		emit_sound ( id, CHAN_VOICE , random_float ( 0.0, 1.0 ) <= 0.5 ? gSnarkHunt2Sound : gSnarkHunt3Sound, VOL_NORM, ATTN_NORM, 0, PITCH_NORM );
	}
	sel_modelent2(id, 1)

	engclient_cmd( id, gWeaponCommand );
    ...
    public CSqueak_PrimaryAttack ( const Player )
    {
        ...
        if ( gPlayerAmmo{ Player } && game_started())
        {
            ...
            if ( !get_tr2( 0, TR_AllSolid ) && !get_tr2( 0, TR_StartSolid ) && Fraction > 0.25 )
            {
                ...
                if ( CSqueak_Create ( Player, EndPos, VAngle, Velocity ) )
                {
                    ...
                    gPlayerAmmo{ Player }--;
                    cs_set_user_bpammo( Player, CSW_FLASHBANG, cs_get_user_bpammo(Player, CSW_FLASHBANG)-1 )

                    if (gPlayerAmmo{ Player } <= 0)
                         sel_modelent2(Player, 0)
    ...
    public CSqueak_Killed ( const Snark, const Killer, const bool:ShouldGib )
    {
        ...
        if (is_user_connected(Killer) && is_user_connected(pev( Snark, pev_RealOwner )) && pev( Snark, pev_RealOwner ) != Killer && !is_user_zombie(Killer))
        	cs_set_user_money_ul( Killer, cs_get_user_money_ul( Killer )+get_pcvar_num( SnarkKilled ) )
    }
    ...
    public CSqueak_HuntThink ( const Snark )
    {
        ...
        if ( CurrentTime >= DieDelay || !game_started())
        {
        ...
        if ( ( Enemy = pev( Snark, pev_enemy ) ) == NULL_ENT || !is_user_alive( pev( Snark, pev_enemy ) ) || is_user_zombie( pev( Snark, pev_enemy ) ) )
        {
...
public maxhumans() {
	...
	{
		if (is_user_connected(i)) {
			if (is_user_alive(i) && !is_user_zombie(i)) count++
		}
	}
    ...
    public CSqueak_SuperBounceTouch ( const Snark, const Other )
    {
        ...
        if ( 1 <= Other <= gMaxClients && pev( Other, pev_takedamage ) && NextAttack < CurrentTime )
        {
            ...
            if ( Hit == Other && pev( Hit, pev_modelindex ) != pev( Snark, pev_modelindex ) && 1 <= Other <= gMaxClients )
            {
                ...
                if ( !is_user_zombie( Other ) && maxhumans() && !is_terminator(Other))
                {

				new armor
				armor = get_user_armor(Other)
				if (armor > 0) {
                    set_pev( Snark, pev_dmg, floatclamp(pev( Snark, pev_dmg ) + (is_alien(Owner) ? get_pcvar_float( pCvarDamagePop )*2 : get_pcvar_float( pCvarDamagePop )), 1.0, (is_alien(Owner) ? 35.0 : 20.0)) );

                    static Float:Damage;
                    pev( Snark, pev_dmg, Damage );

                    armor -= floatround(Damage)

                    if(armor < 0)
                        armor = 0
                    set_user_armor(Other,armor)
					} else {
                    static Float:Forward    [ Coord_e ];
                    static Float:EndPos     [ Coord_e ];
                    ...
                    if (!is_user_alive(Other))
                    	cs_set_user_money_ul( Owner, cs_get_user_money_ul( Owner )+(is_alien(Owner) ? get_pcvar_num( SnarkKill )*2 : get_pcvar_num( SnarkKill )) )
                    }
                }
                else if (!is_user_zombie( Other ) && !is_terminator(Other))
                {
					new armor
					armor = get_user_armor(Other)
					...
					if (armor > 0) {
						...
						infect_user(Other, Owner)
						if ( Owner != Other ) {
							make_deathmsg(Owner, Other, 0, "infect")
							set_user_frags( Owner, get_user_frags( Owner ) + 1 );
							cs_set_user_money_ul( Owner, cs_get_user_money_ul( Owner )+get_pcvar_num( SnarkInfect ) )
						}
    ...
    CSqueak_Spawn ( const Player, const Snark, const Float:Origin[ Coord_e ] )
    {
        ...
        set_pev( Snark, pev_health, (is_alien(Player) ? get_pcvar_float( pCvarHealth )*3 : get_pcvar_float( pCvarHealth )) + gSnarkHealthReference );
        set_pev( Snark, pev_gravity, get_pcvar_float( pCvarGravity ) );
        set_pev( Snark, pev_friction, get_pcvar_float( pCvarFriction ) );
        //set_pev( Snark, pev_maxspeed, 3500 );
    ...
    UTIL_BestVisibleEnemy ( const Snark, const Float:DistanceToSearch /* , const Flags */ )
    {
        ...
        if (is_user_zombie( ReturnEntity ))
			return false
        else
    ...
    stock UTIL_RadiusDamage ( const Float:Origin[ Coord_e ], const Inflictor, const Attacker, const Float:Damage, const Float:Radius, const DamageBits )
    {
        ...
        while ( ( Entity = find_ent_in_sphere( Entity, Origin, Radius ) ) != NULL_ENT )
        {
            ...
            if ( pev( Entity, pev_takedamage ) && is_user_connected( Entity ) && !is_user_zombie( Entity ) )
            {
